...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'spendwise-dev-secret-change-me')

# Vercel serverless functions need a writable temporary directory when a
# DATABASE_URL is not supplied. For real production data, set DATABASE_URL
# to a managed PostgreSQL database (for example Neon) in Vercel.
database_url = os.getenv('DATABASE_URL')
if database_url:
    database_url = database_url.replace('postgres://', 'postgresql://', 1)
    app.config['SQLALCHEMY_DATABASE_URI'] = database_url
else:
    db_path = '/tmp/spendwise.db' if os.getenv('VERCEL') else 'spendwise.db'
...
def dashboard():
    m = month_key()
    expenses = expense_total(current_user.id, m)
    income = income_total(current_user.id, m)
...
def expenses():
    ...
    month = request.args.get('month', month_key())
    if search:
        q = q.join(Category).filter((Expense.note.ilike(f'%{search}%')) | (Category.name.ilike(f'%{search}%')))
    if month:
        q = q.filter(func.strftime('%Y-%m', Expense.spent_on) == month)
...
def budget():
    if request.method == 'POST':
        try:
            ...
            b = Budget.query.filter_by(user_id=current_user.id, category_id=category_id, month=month).first()
            if b:
                b.amount = amount
            else:
                db.session.add(Budget(user_id=current_user.id, category_id=category_id, month=month, amount=amount))
            db.session.commit()
            flash('Budget saved.', 'success')
            return redirect(url_for('budget'))
        except (ValueError, KeyError):
            flash('Invalid budget details.', 'danger')
...
def family():
    if request.method == 'POST':
        try:
            ...
            flash('Invalid monthly limit.', 'danger')
...
def reports():
    m=month_key()
    inc=income_total(current_user.id,m)
    exp=expense_total(current_user.id,m)
    return render_template('reports.html', month=m, income=inc, expenses=exp, balance=inc-exp)
...
def api_summary():
    ...
    inc=income_total(current_user.id,m)
    exp=expense_total(current_user.id,m)
    return jsonify({'income':inc,'expenses':exp,'balance':inc-exp})
